...
const fetchLyrics = async (lyricsUrl?: string): Promise<string> => {
  if (!lyricsUrl) {
    throw new Error("Lyrics URL is missing");
  }
...
const getAlternativeUrl = (url: string): string => {
  return url;
};
...
const SongCard: React.FC<SongCardProps> = ({ song, index }) => {
  ...
  const handleAudioError = () => {
    if (audioFailedOver) {
      console.error(`Audio totally failed to load for: ${song.title}`);
      setAudioError(true);
      return;
    }
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs font-mono text-[#707075]">
                <span className="text-[#F27D26]">
                  {song.description || "Инструментальный фон"}
                </span>
                <span>•</span>
                <span>MP3 FILE</span>
              </div>
          ...
          {audioError ? (
            ...
            <div className="w-full flex flex-col gap-1.5">
              <div className="flex justify-between items-center px-1 mb-1">
                <span className="text-[#E0E0E0] text-xs font-mono font-medium">
                  {song.title}.mp3
                </span>

                <div className="flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping" />
                  <span className="text-[#707075] text-[10px] font-mono uppercase tracking-wider">
                    Ready / 320kbps
                  </span>
                </div>
              </div>
